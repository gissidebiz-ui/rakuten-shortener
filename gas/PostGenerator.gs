...
・今日は【 ${todayStr} 】です。指定された曜日（平日・週末など）のリアルなタイム感と、読者の心理状態（例：日曜の夜の憂鬱、月曜の朝の気怠さ等）に完全に合致した内容にしてください。
・【厳禁】実際の曜日や日付と矛盾する表現（例：日曜日なのに「今週も始まったね」、月初なのに「月末」等）は絶対に出力しないでください。
・【最重要タイムコンテキスト】現在時刻は【 ${timeContext} 】であると仮定してください。今まさにこの時間帯に投稿している体裁で、その時間帯特有のターゲットの状況（例：朝の通勤時の憂鬱、昼休みの息抜き、夕方の疲労など）に完全に寄り添った内容にしてください。
・【厳禁】指定された時間帯【 ${timeContext} 】と少しでもずれた時間的表現（朝の枠なのに「もうすぐお昼」、夕方の枠なのに「おはよう」「こんばんは」等）は極めて不自然になるため絶対に生成しないでください。
・投稿本文のみを出力してください。
...
【構成指示】
・【最重要：Meta社「詐欺とスパム」規約の厳守】
ユーザーを欺く表現や、過剰なクリックベイト（Engagement Bait）は絶対に行わないでください。以下のルールを厳守すること。
